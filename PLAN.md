...
- [~] Cross-node edges over `NativeTransport` (and over `SshTransport` via stdin forwarding for simple two-node cases); EOF/half-close/exit propagation; `pipefail` semantics; `--ordered` merge; `spool` sink; `--idempotent` stage flag gating retries. _(NativeTransport cross-node edges + node stdin forwarding + EOF/exit propagation + pipefail done (`DistributedRunner`, `pipe` remote). **Remaining:** SshTransport two-node, `--ordered`, `spool`, `--idempotent`, cross-stream backpressure, downstream-exit SIGPIPE to upstream.)_
