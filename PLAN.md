...
- [~] `pipeshellx node` agent: listener, job supervisor (lease expiry kills jobs), `AF_UNIX`/named-pipe local endpoint, metrics endpoint; service install for systemd/launchd/SCM. _(listener ✓, lease/fencing job supervisor ✓, **service install ✓** — `node systemd-unit` (restart + hardening: NoNewPrivileges/ProtectSystem/PrivateTmp/…) and `node launchd-plist` emit ready-to-install units built from the daemon's flags; the plist passes `plutil -lint`. **Remaining:** AF_UNIX local endpoint + metrics endpoint; SCM (Windows, with Phase 3).)_
