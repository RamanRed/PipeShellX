...
- [~] `pipeshellx pipe` with `'cmd'@placement` stages and `'|'` edges; `pipeline.yaml` for general DAGs (fan-in/fan-out, named edges); `Planner` validates acyclicity and placement. _(`pipe` (parser→Planner→run), local + all-remote, done; Planner validates acyclicity/edges; placement resolves via the inventory. **Remaining:** `pipeline.yaml`.)_
- [~] Cross-node edges over `NativeTransport` (and over `SshTransport` via stdin forwarding for simple two-node cases); EOF/half-close/exit propagation; `pipefail` semantics; `--ordered` merge; `spool` sink; `--idempotent` stage flag gating retries. _(NativeTransport cross-node edges + node stdin forwarding + EOF/exit propagation + pipefail + **mixed local/remote splicing** (`@local`) + **group fan-in** (`@group`, `FanInPipeline`) done (`DistributedRunner`/`LocalRunner`/`SegmentedPipeline`/`FanInPipeline`, `pipe`). **Remaining:** SshTransport two-node, `--ordered`, `spool`, `--idempotent`, cross-stream backpressure.)_
- [~] `ConsensusEngine` as a reduce sink (`diff` subcommand, `--consensus`); `--canary`, `--fail-fast`. _(`ConsensusEngine` (`psx::sink::consensus` — groups hosts by identical output, majority-first, outliers) + `pipeshellx diff` over the native backplane done; exit 0 unanimous / 1 drift / 2 host failure. **Remaining:** `--consensus` flag on run/pipe, `--canary`, `--fail-fast`.)_
