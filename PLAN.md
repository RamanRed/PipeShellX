...
- [~] Cross-node edges over `NativeTransport` (and over `SshTransport` via stdin forwarding for simple two-node cases); EOF/half-close/exit propagation; `pipefail` semantics; `--ordered` merge; `spool` sink; `--idempotent` stage flag gating retries. _(NativeTransport cross-node edges + node stdin forwarding + EOF/exit propagation + pipefail + **mixed local/remote splicing** (`@local`) done (`DistributedRunner`/`LocalRunner`/`SegmentedPipeline`, `pipe`). **Remaining:** SshTransport two-node, `--ordered`, `spool`, `--idempotent`, cross-stream backpressure.)_
